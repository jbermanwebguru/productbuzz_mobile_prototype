    ...
    jQTouchCore = function(options) {
        ...
        function addPageToHistory(page, animation) {
			...
			for(var i=0; i<history.length; i++) {
				//console.log("history["+i+"]="+history[i].hash);
			}
        }
        ...
        function doNavigation(fromPage, toPage, animation, goingBack) {
			...
			console.log("doNavigation. fromPage="+fromPage[0].id+" toPage="+toPage[0].id);
			
            goingBack = goingBack ? goingBack : false;

            // Error check for target page
            if (toPage === undefined || toPage.length === 0) {
                console.log("cancelling doNavigation. "+(toPage === undefined ? "toPage === undefined" : (toPage.length === 0 ? "toPage.length === 0" : "")));
                $.fn.unselect();
                return false;
            }
            ...
            if (toPage.hasClass('current')) {
                console.log("cancelling doNavigation. toPage.hasClass('current').");
                $.fn.unselect();
                return false;
            }
            ...
            if ($.support.animationEvents && animation && jQTSettings.useAnimations) {
				...
				//JBB/MKTG: implemented fix from https://github.com/senchalabs/jQTouch/issues/213 to ensure event fires
                setTimeout(function() {navigationEndHandler()}, 250);
            ...
            function navigationEndHandler(event) {
				...
				console.log("navigationEndHandler. goingBack="+goingBack+" currentPage="+$currentPage.attr('id'));
        ...
        function goBack() {
			...
			console.log("goBack. history.length="+history.length+" from.page="+from.page[0].id+" to.page="+to.page[0].id);
			
            if (doNavigation(from.page, to.page, from.animation ? from.animation : {name:'slideleft', selector:'.slideleft, .slide, #jqt > * > ul li a'}, true)) {
                return publicObj;
            } else {
                return false;
            }
        ...
        function goTo(toPage, animation) {
            ...
            if (typeof toPage === 'string') {
				console.log("goTo. passed toPage="+toPage);
                var nextPage = $(toPage);
			...
			console.log("goTo. final toPage="+toPage[0].id);
        ...
        function hashChangeHandler(e) {
            console.log("jqtouch.hashChangeHandler: location.hash="+location.hash)
            if (location.hash === history[0].hash) {
                ...
                if( (history[1] && location.hash === history[1].hash) ) {
                    console.log("jqtouch: empty history, going back")
                    goBack();
                    return true;
                } else {
        ...
        function setHash(hash) {
            // Sanitize
            console.log("setHash. previously: "+location.hash+" hash="+hash);
            location.hash = '#' + hash.replace(/^#/, '');
        }
        function showPageByHref(href, options) {
			
			console.log("showPageByHref. href="+href);
        ...
        function touchStartHandler(e){
            ...
            if ($el.length && $el.attr('href') && !$el.hasClass('disabled')) { //added 'disabled' check, JBB/MKTG
                $el.addClass('active');
            }
        ...
        function tapHandler(e){
            ...
            if ($el.isExternalLink()) {
                ...
                console.log("backSelector tapped");
                goBack(hash);
        ...
        $(document).ready(function RUMBLE() {
			...
			//console.log("startHash 1="+startHash+" $('#jqt > .current').length="+$('#jqt > .current').length);
            ...
            // ADDED by JBB/MKTG, set ID if not already set
            //if(!$currentPage.attr('id'))
            //    $currentPage.attr('id',(startHash ? startHash : 'first-page'));
            // DONE ADD

            console.log("startHash="+startHash+" $currentPage.attr('id')="+$currentPage.attr('id'));

            if($currentPage.attr('id')) {
                setHash($currentPage.attr('id'));
                addPageToHistory($currentPage);
            }
			//console.log("startHash 2="+startHash);
			
			if(startHash !== "") {
				if ($(startHash).length === 1) {
					goTo(startHash);
                } else {
                    jQT.goTo(startHash.substring(1)+".html");
                    // tried using the json object to match up the page to its url, it got convulted with the 3 sections each topic could correspond to
				/*
                } else if(typeof topicsArray != "undefined") { // ADDED by JBB/MKTG, load ajax page from hash via topicsArray object definited in application.js
					for(var i=0; i<topicsArray.length; i++) {
						console.log("startHash.substring(1)="+startHash.substring(1)+" topicsArray["+i+"].id="+topicsArray[i].id);
						if(startHash.substring(1) == topicsArray[i].id)
							jQT.goTo(topicsArray[i].url);
                        else {
                            for(var h=0; h<topicsArray[i].children.length; h++) {
                                console.log("startHash.substring(1)="+startHash.substring(1)+" topicsArray["+i+"].children["+h+"]="+topicsArray[i].children[h].id);
                                if(startHash.substring(1) == topicsArray[i].id+"-"+topicsArray[i].children[h])
                                    jQT.goTo(topicsArray[i].children[h].url);
                            }
                        }
					}
                    */
